	...
	protected function getForm() {
		...
		if (isset($this->request->post['config_comment'])) {
			...
			$data['config_comment'] = '';
				...
				$data['text_structured_data'] = 'Structured Data';
				$data['text_structured_data_info'] = 'Structured data added to your site: store name, site url, site logo(if set), Google Sitelinks search box, social profile links (if set), customer service telephine set to international format (if set, Google will display it only if the phone is in this format)';
				$data['entry_support'] = 'Custumer Support Telephone';
				$data['help_support'] = 'An internationalized version of the site number, starting with the + symbol and country code (ex. +44-2078225951)';
				if (isset($this->request->post['config_support'])) {
			        $data['config_support'] = $this->request->post['config_support'];
		        } elseif (isset($store_info['config_support'])) {
		        	$data['config_support'] = $store_info['config_support'];
		        } else {
	        		$data['config_support'] = '';
        		}
				$data['entry_social'] = 'Social Profile Links';
				$data['help_social'] = 'Comma separated! Add your profiles to the Google Knowledge panel (Google+, Facebook, Twitter, Instagram, YouTube, LinkedIn, Myspace, Pinterest, SoundCloud, Tumblr).';
				if (isset($this->request->post['config_social'])) {
			       $data['config_social'] = $this->request->post['config_social'];
		        } elseif (isset($store_info['config_social'])) {
			       $data['config_social'] = $store_info['config_social'];
		        } else {
			       $data['config_social'] = '';
		        }
