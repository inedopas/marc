...
<a id="button-filter-open" class="btn btn-filter" onclick="open_filter(this);return false;"><i class="fa fa-filter"></i> Фильтры</a>
<div id="mfilter-content-container">
  		...
  		<div class="row">
  			...
  			<div class="name-actions col-sm-4">
  				...
  				<div class="price">
  					...
  					<?php echo "от " . $product['price'] . " ₽"; ?>
  					<?php } else { ?>
  					<span class="price-old"><?php echo "от " . $product['price'] . " ₽"; ?></span> <span class="price-new"><?php echo "от " . $product['special'] . " ₽"; ?></span>
  					<?php } ?>
  				</div>
  				<?php } ?>
  				...
  				<ul>
  				     <?php if($theme_options->get( 'display_add_to_cart' ) != '0') { ?>
  				          ...
  				          <?php } else { ?>
  				          <li><a onclick="cart.add('<?php echo $product['product_id']; ?>');" data-toggle="tooltip" data-original-title="<?php echo $button_cart; ?>"><i class="fa fa-shopping-basket"></i></a></li>
  				          <?php } ?>
  				     <?php } ?>
  					...
  					<li><a onclick="compare.add('<?php echo $product['product_id']; ?>');" data-toggle="tooltip" data-original-title="<?php if($theme_options->get( 'add_to_compare_text', $config->get( 'config_language_id' ) ) != '') { echo $theme_options->get( 'add_to_compare_text', $config->get( 'config_language_id' ) ); } else { echo 'Add to compare'; } ?>"><i class="fa fa-bar-chart"></i></a></li>
  					<?php } ?>
  ...
  ?>
  <div class="category_wrapper product-grid"<?php if($theme_options->get('default_list_grid') == '1') { echo ' class="active"'; } ?>>
  	<div class="row">
	  	<?php $row_fluid = 0; foreach ($products as $product) { $row_fluid++; ?>
		  	<?php $r=$row_fluid-floor($row_fluid/$row)*$row; if($row_fluid>$row && $r == 1) { echo '</div><div class="row">'; } ?>
		  	<div class="<?php if ($product_in_row_class) { echo 'col-xs-' . $product_in_row_class; } else { ?>col-sm-<?php echo $class; ?> col-xs-6<?php } ?>">
...
$('#filters_button_mobile').click(function(){
	if ($('#mfilter-box-1').hasClass('active-filter')) {
		$('#mfilter-box-1').removeClass('active-filter');
	} else {
		$('#mfilter-box-1').addClass('active-filter');
	}
});
	$('#filters_button_mobile_clouse, .mfilter-option, .mfilter-button-reset').click(function(){
		$('#mfilter-box-1').removeClass('active-filter');
	});
//--></script>
</div>
