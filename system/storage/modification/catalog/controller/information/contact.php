...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$msg_body = '
			' . $this->language->get('text_customer_name') . ' ' . $this->request->post['name'] . '

			' . $this->language->get('text_email') . ' ' . strip_tags(html_entity_decode($this->request->post['email'], ENT_QUOTES, 'UTF-8')) . '

			' . $this->language->get('text_enquiry') . ' ' . strip_tags(html_entity_decode($this->request->post['enquiry'], ENT_QUOTES, 'UTF-8')) . '
						
			' . $this->language->get('text_time') . ' ' . date('d.m.Y H:i');
			
			$mail->setText(html_entity_decode($msg_body, ENT_QUOTES, 'UTF-8'));
